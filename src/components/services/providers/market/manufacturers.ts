...
export const ManufacturersProvider = new class implements IFetchProvider{
    ...
    async add(data: IData): Promise<Response_ID> {
        const res = await fetchJSON<Response_ID>('POST', statics.host.api + '/api/v1/items/manufacturers/add', data)
        return { id: res ? res.id : 0 }
    }

    async edit(data: IData): Promise<any> {
        const res = await fetchJSON('POST', statics.host.api + '/api/v1/items/manufacturers/add', data)
        return res
    }
    ...
    async list(req: Params_Paged): Promise<Response_List> {
        ...
        const res = await fetchJSON<Response_List>('POST', statics.host.api + '/api/v1/items/manufacturers/list', {
            offset: page * perpage,
            limit: perpage,
            count: true,
        })
    ...
    async delete(id: number): Promise<void> {
        await fetchJSON('DELETE', statics.host.api + '/api/v1/items/manufacturers/delete', { id })
        return
    }
}